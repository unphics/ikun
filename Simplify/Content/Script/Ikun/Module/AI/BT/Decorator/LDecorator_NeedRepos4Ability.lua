...
function LDecorator_NeedRepos4Ability:Judge()
    ...
    if not hasObs then
        log.log(log.key.repos..rolelib.roleid(self.Chr)..'LDecorator_NeedRepos4Ability : 没阻挡！')
        return false
    end
    log.log(log.key.repos..rolelib.roleid(self.Chr)..'LDecorator_NeedRepos4Ability : 阻挡！！！')
    return true
end
---@private
