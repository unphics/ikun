...
function BP_Chr_Katana:ReceiveBeginPlay()
    ...
    if self:HasAuthority() then
        ...
        local handle = self.ASC:K2_GiveAbility(ability_normal, 1, 1)
        self.ability_handles.normal = handle
    end
...
BP_Chr_Katana.NormalAttack = function(self)
    ...
    log.log('BP_Chr_Katana.NormalAttack')

    self:GetAbilitySystemComponent():TryActivateAbility(self.ability_handles.normal)
end
